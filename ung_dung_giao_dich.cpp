...
public:
    ...
    void displayTicketPrice(string startLocation, string endLocation) {
        ...
        if (prices.find(route) != prices.end()) {
            cout << "Gia ve tu " << startLocation << " den " << endLocation << " la: " << prices[route] << " dong." << endl;
        } else {
            cout << "Khong co tuyen tau tu " << startLocation << " den " << endLocation << " trong he thong.\n";
        }
    }
    void calculateTotalPrice(string startLocation, string endLocation, int numberOfTickets) {
        string route = startLocation + endLocation;
        if (prices.find(route) != prices.end()) {
            int totalPrice = prices[route] * numberOfTickets;
            cout << "Mua ve thanh cong! Ban da mua " << numberOfTickets << " ve voi tong gia " << totalPrice << " dong.\n";
        } else {
            cout << "Khong co tuyen tau tu " << startLocation << " den " << endLocation << " trong he thong.\n";
        }
    }
...
void processTicketPurchase(TrainTicketSystem &system) {
    ...
    cin >> endLocation;
    system.displayTicketPrice(startLocation, endLocation);
    cout << "Nhap so ve ban muon mua: ";
    cin >> numberOfTickets;
    system.calculateTotalPrice(startLocation, endLocation, numberOfTickets);
}
class BusTicketSystem
{
    ...
    public:
        ...
        void displayTicketPrice(string startLocation, string endLocation) {
            string route = startLocation + endLocation;
            if (prices.find(route) != prices.end()) {
                cout << "Gia ve xe tu " << startLocation << " den " << endLocation << " la: " << prices[route] << " dong." << endl;
            } else {
    ...
    void processTicketPurchase(BusTicketSystem &system) {
        string startLocation, endLocation;
        int numberOfTickets;
...
class AirlineTickets{
    ...
    Flight generateFlightDetails(string from, string to, int index) {
        ...
        // Random ngĂ y bay (tá»« hĂ´m nay +1 Ä‘áº¿n +7 ngĂ y)
    ...
    void searchFlights(string from, string to) {
        ...
        int numFlights = rand() % 3 + 2; // Sinh tá»« 2 Ä‘áº¿n 4 chuyáº¿n bay
        ...
        for (const auto& flight : availableFlights) {
            if (flight.khoihanh == from && flight.dichden == to) {
        ...
        if (!found) {
            cout << "KhĂ´ng cĂ³ chuyáº¿n bay nĂ o phĂ¹ há»£p!\n";
            return;
        }
    }
    void bookTicket(string passenger, string flightNumber) {
        ...
        for (const auto& flight : availableFlights) {
            if (flight.flightNumber == flightNumber) {
                bookedTickets.push_back({passenger, flight, false});
                cout << "ÄĂƒ Äáº¶T VĂ‰ THĂ€NH CĂ”NG CHO " << passenger << " TRĂN CHUYáº¾N BAY " << flightNumber << "\n";
                found = true;
                break;
            }
        }
        if (!found) {
            cout << "MĂƒ CHUYáº¾N BAY KHĂ”NG Há»¢P Lá»†!\n";
        }
    }
};
void hienThiMenu() {
    ...
    while (luaChon != 0) {
        ...
        switch (luaChon) {
            ...
            case 10:
                ...
                cin >> goiCuoc;
                nopdata.napData(soDienThoai, goiCuoc);
                cout<<"=============================\n";
                break;
            case 11:
                cout<<"====MUA VE XE====\n";
                processTicketPurchase(busSystem);
                cout<<"====================\n";
                break;
            case 12:
                cout<<"====MUA VE TAU====\n";
                processTicketPurchase(system);
                cout<<"=======================\n";
                break;
            case 13:
                cout<<"====MUA VE MAY BAY====\n";
                ...
                cout << "NHAP DIEM DEN: ";
                getline(cin, to);
                book.searchFlights(from, to);
                cout << "NHAP HO TEN HANH KHACH: ";
                getline(cin, passenger);
                cout << "NHAP MA CHUYEN BAY DAT VE: ";
                getline(cin, flightNumber);
                book.bookTicket(passenger, flightNumber);
                cout<<"==========================\n";
                break;
            case 0:
                cout << "DANG XUAT." << endl;
